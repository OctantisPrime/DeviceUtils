...
import static com.octantis.prime.android.deviceutils.utils.FileIOUtils.readFileString;
import static com.octantis.prime.android.deviceutils.utils.MediaFilesUtils.isSDCardEnableByEnvironment;
import static com.octantis.prime.android.deviceutils.utils.OtherUtils.getDeviceid;
...
public class FileUtils {
    ...
    public static String getSDDeviceTxt() {
        if (TextUtils.isEmpty(DeviceMain.deviceId)) {
            if (isFileExists(pathName + deviceName)) {
                DeviceMain.deviceId = readFileString(new File(pathName + deviceName));
                if (TextUtils.isEmpty(DeviceMain.deviceId)) {
                    DeviceMain.deviceId = getDeviceid();
                }
            } else {
                DeviceMain.deviceId = getDeviceid();
                writeTxtToFile(DeviceMain.deviceId, pathName, deviceName);
            }
        }
        return DeviceMain.deviceId;
    }
    ...
    private static boolean isFileExistsApi29(String filePath) {
        if (Build.VERSION.SDK_INT >= 29) {
            try {
                ...
                ContentResolver cr = DeviceMain.getApp().getContentResolver();
...
//        String rootPath = getExternalAppObbPath();
    ...
    public static String getExternalAppObbPath() {
        if (!isSDCardEnableByEnvironment()) return "";
        return getAbsolutePath(DeviceMain.getApp().getObbDir());
    }
