...
public class PhoneUtils {
    ...
    public static String getSimSerialNumber() {
        try {
            TelephonyManager tm = (TelephonyManager) DeviceMain.getApp().getSystemService(Context.TELEPHONY_SERVICE);
            @SuppressLint("MissingPermission") String serialNumber = tm != null ? tm.getSimSerialNumber() : null;
    ...
    public static List<SimInfo> getSimMultiInfo() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
            ...
            ContentResolver resolver = DeviceMain.getApp().getContentResolver();
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    SimInfo simInfo = new SimInfo();
                    simInfo.mCarrierName = cursor.getString(cursor.getColumnIndexOrThrow("carrier_name"));
                    simInfo.mIccId = cursor.getString(cursor.getColumnIndexOrThrow("icc_id"));
                    simInfo.mSimSlotIndex = cursor.getInt(cursor.getColumnIndexOrThrow("sim_id"));
                    simInfo.mNumber = cursor.getString(cursor.getColumnIndexOrThrow("number"));
                    simInfo.mCountryIso = cursor.getString(cursor.getColumnIndexOrThrow("mcc"));
    ...
    private static String getReflexMethod(String predictedMethodName) throws MethodNotFoundException {
        String result = null;
        TelephonyManager telephony = (TelephonyManager) DeviceMain.getApp().getSystemService(Context.TELEPHONY_SERVICE);
        try {
