...
import static com.octantis.prime.android.deviceutils.utils.OtherUtils.getJudgeSIMCount;
...
import com.octantis.prime.android.deviceutils.data.SimCardData;
...
public class GeneralUtils {
    ...
    public static boolean isChekSelfPermission(String permission) {
        if (ActivityCompat.checkSelfPermission(DeviceMain.getApp(), permission)
    ...
    public static String getNetworkType() {
        ...
        ConnectivityManager cm =
                (ConnectivityManager) DeviceMain.getApp().getSystemService(Context.CONNECTIVITY_SERVICE);
        if (cm == null) {
            return null;
        }
    ...
    private static boolean isEthernet() {
        ...
        @SuppressLint("MissingPermission") final NetworkInfo info = cm.getNetworkInfo(ConnectivityManager.TYPE_ETHERNET);
    ...
    public static SimCardData getSimCardInfo() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
            //1.版本超过5.1，调用系统方法
            SubscriptionManager mSubscriptionManager = (SubscriptionManager) DeviceMain.getApp().getSystemService(Context.TELEPHONY_SUBSCRIPTION_SERVICE);
    ...
    public static String getCidNumbers() {
        if (getTelephonyManager().getPhoneType() == PHONE_TYPE_GSM) {
            if (ActivityCompat.checkSelfPermission(DeviceMain.getApp(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                return "";
            }
            @SuppressLint("MissingPermission") final GsmCellLocation location = (GsmCellLocation) getTelephonyManager().getCellLocation();
            if (location != null) {
    ...
    public static String getMyUUID() {
        ...
        androidId = "" + Settings.Secure.getString(DeviceMain.getApp().getContentResolver(),
                Settings.Secure.ANDROID_ID);
    ...
    public static void getGaid() {
        Executors.newSingleThreadExecutor().execute(() -> {
            try {
                gaid = AdvertisingIdClient.getGoogleAdId(DeviceMain.getApp());
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    ...
    public static String getUiModeType() {
        UiModeManager mUiModeManager = (UiModeManager) DeviceMain.getApp().getSystemService(Context.UI_MODE_SERVICE);
